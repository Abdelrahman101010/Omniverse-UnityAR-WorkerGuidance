...
async def export_part_glb(part: PartSpec, output_dir: Path) -> tuple[bool, str]:
    ...
    print(f"[{part.step_id}] Wrote {output_glb.name} ({size_kb} KB)")
    return True, str(output_glb)


async def run() -> None:
    output_dir = REPO_ROOT / "shared" / "samples" / "assets" / "_raw" / JOB_ID
    ...
    for part in PARTS:
        ok, detail = await export_part_glb(part, output_dir)
        report.append({
            "step_id": part.step_id,
            "part_id": part.part_id,
            "display_name": part.display_name,
            "sequence_index": part.sequence_index,
            "glb_file": f"{part.part_id}.glb" if ok else None,
            "ok": ok,
            "detail": detail,
        })
...
# Schedule the coroutine on Kit's event loop so this script works whether it's
# executed from the Script Editor or via `kit.exe --exec`.
